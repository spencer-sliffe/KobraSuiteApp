...
def apply_task_rewards(module_task):
    currency, experience, population = None, None, None
    profile = getattr(module_task.user, 'profile', None)
    if not profile:
        return
    currency = currency_gain = math.floor(random.randint(5, 10) * profile.multiplier()) * module_task.task_weight
    add_currency_to_wallet(profile, currency_gain)
    if module_task.task_number != 0:
        experience = experience_gain = module_task.task_weight * profile.multiplier()
        update_experience(profile, module_task.module, experience_gain)
        population = update_streak_and_population(profile, module_task.module)
    return currency, experience, population
