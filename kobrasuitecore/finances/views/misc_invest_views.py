...
class MiscInvestViewSet(viewsets.ViewSet):
    ...
    @action(detail=False, methods=['get'])
    def hot_crypto(self, request):
        user = request.user
        budget = 0
        if hasattr(user, 'finance_profile'):
            budget = user.finance_profile.budget
        cryptos = get_hot_crypto(budget)
        if not cryptos:
            return Response({"message": "No cryptos found."}, status=status.HTTP_200_OK)
        return Response(cryptos, status=status.HTTP_200_OK)

    @action(detail=False, methods=['get'])
    def crypto_data(self, request):
        ...
        if not data:
            return Response({"error": f"No data found for {cid}"}, status=status.HTTP_404_NOT_FOUND)
        return Response(data, status=status.HTTP_200_OK)
    ...
    @action(detail=False, methods=['get'])
    def crypto_news(self, request):
        key = os.environ.get('NEWS_API_KEY')
        if not key:
            return Response({"error": "No NEWS_API_KEY found"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
        q = request.query_params.get('q', 'cryptocurrency')
        pg = request.query_params.get('page', 1)
        try:
            pg = int(pg)
        except:
